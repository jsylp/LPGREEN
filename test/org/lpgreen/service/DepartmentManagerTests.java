...
public class DepartmentManagerTests extends AbstractTransactionalDataSourceSpringContextTests {
	...
	public void testAddAndUpdateDepartment() {
		...
		dept.setName("LPGREEN3");
		dept.setDescription("Develop LPGREEN3 App");
		dept.setDeptHead(3);
		...
		try {
			...
			assertEquals(retDept.getName(), "LPGREEN3");
			assertEquals(retDept.getDescription(), "Develop LPGREEN3 App");
		}
		catch (Exception e) {
			fail(e.getMessage());
		}
		...
		try {
			retDept.setDescription("Develop LPGREEN3 schedule change");
			
			Department retDeptUpd = departmentManager.updateDepartment(
					currentUserId, retDept);
			assertNotNull(retDeptUpd);
			assertEquals(retDeptUpd.getDescription(), "Develop LPGREEN3 schedule change");
		}
		catch (Exception e) {
			fail(e.getMessage());
		}
